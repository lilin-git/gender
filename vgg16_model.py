...
import numpy as np
from gender.loads import load_dataset
...
def label_onehot(labels):
    oh = tf.one_hot(indices=labels, depth=2, on_value=1.0, off_value=0.0, axis=-1)
    return oh



def vgg16_model_fn(features, labels, mode):
    input_layer = tf.reshape(features, (-1, 224, 224, 3))
    logits = inference_op(input_op=input_layer, keep_prob=keep_prob)
    predictions = {
...
if __name__ == "__main__":
     image_batch, label_batch = load_dataset("data/train/")
     image_batch = tf.reshape(image_batch, (-1, 224, 224, 3))
     label_batch = label_onehot(label_batch)
     test_image, test_label = load_dataset("data/test/")
     test_image = tf.reshape(test_image, (-1, 224, 224, 3))
     test_label = label_onehot(test_label)
